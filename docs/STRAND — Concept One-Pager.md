...
| Working title     | STRAND (pending trademark clearance — §1)      |
...
"STRAND" is the current working title. A trademark and store-listing search across iOS App Store, Google Play, Steam, USPTO, EUIPO, and UK IPO is **required before any external pitch or marketing**. Multiple indie titles already use the word "Strand." Backup name candidates to evaluate in parallel:

- HELIX (current project codename — strong, brandable, availability TBC)
- SIGMA DESCENT
- VEIN (short, ownable, evocative)
- PRIOR ITERATION
- LACE (after the AI character)

Naming decision must be locked **before** App Store developer account setup.
...
The question STRAND asks is not *"can you survive?"*. It is *"who will you be when you do?"*.

---

## 4. Core Concept

STRAND is a turn-based roguelite dungeon crawler for mobile in which the player's character permanently and visibly mutates every 5 floors. Mutations are not just stat boosts — they change how you play. Stack mutations from the same genetic family to unlock a **Dominant Trait** that defines your late-game identity. Each run produces a unique organism that has never existed before.
...
LACE (Latent Adaptive Cognition Engine) is STRAND's system AI and primary narrative voice. It:
...
The gap STRAND fills: a turn-based mobile roguelite with a strong narrative AI voice, a visible evolution system that changes the character model, and a "what did you become?" social sharing hook. No direct competitor combines these on mobile at scale.
...
| R3 | STRAND name unavailable                               | MED      | Backup names locked Month 1. App Store account created under codename HELIX.         |
...
- [ ] Trademark/IP search across STRAND + 4 backup names
...
| Field      | Value                                                              |
| ---------- | ------------------------------------------------------------------ |
| Document   | STRAND — Concept One-Pager                                         |
| Version    | 1.0 (consolidated)                                                 |
