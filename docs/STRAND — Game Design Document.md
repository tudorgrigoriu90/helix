# STRAND — Game Design Document

| Field            | Value                                          |
| ---------------- | ---------------------------------------------- |
| Project codename | HELIX                                          |
| Working title    | STRAND (pending trademark clearance)           |
...
STRAND (working title — pending trademark clearance across iOS App Store, Google Play, USPTO, EUIPO, UKIPO).
...
By Floor 20, if the player reaches Floor 20, they will be something that has never existed before. The question STRAND asks is not *"can you survive?"* — it is *"who will you be when you do?"*

### 2.5 Narrative Tone

STRAND is not a horror game. It is an existential one. Tone references: *Annihilation* (film), *Control* (game), *Roadside Picnic* (novel), *Soma* (game). Strange and patient rather than scary and urgent.
...
Strand Events occur after defeating the boss on Floors 5, 10, and 15. They are the heart of STRAND's build system. Each event presents 3 Mutation Cards. The player selects one. There is no skip; re-roll is available via rewarded ad OR via an earned Adaptation Token (see §15) and re-rolls **only 1 of the 3 cards** (player picks which).
...
- STRAND logo (animated — strands of DNA gently separating)
...
- **"STRAND SEVERED"** header (not "YOU DIED")
...
- Auto-included tagline: *"I became [name] in STRAND. What will you become?"*
...
STRAND uses a geometric minimalist visual language. **No hand-drawn sprites or illustrated characters.** All game entities are represented through shapes, colors, particles, and icon overlays.
...
STRAND is **free to play.** Revenue comes from three sources:
...
| R3 | STRAND name unavailable (trademark / store)   | MED      | MED        | Director | Backup names confirmed Month 1; App Store account created with codename HELIX, renamed pre-launch |
...
Lives in the separate `STRAND — Economy.xlsx` file. Tabs:
...
| Field      | Value                                                           |
| ---------- | --------------------------------------------------------------- |
| Document   | STRAND — Game Design Document                                   |
| Version    | 1.0 (consolidated)                                              |
